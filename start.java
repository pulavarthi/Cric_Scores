...
public class start extends javax.swing.JFrame implements ActionListener {
      ...
      void start()
    {
            ...
            try{
             stmt=connection.con.prepareStatement("update schedule set STATUS=?,SR=? where SR=?");
             stmt.setInt(3, brd);
            ...
            stmt=connection.con.prepareStatement("update "+"score"+brd+" set TEAMS=?,RUNS=?,WICKETS=?,OVERS=?,RR=?,TYPE=? where SN=? ");
            stmt.setString(1, TM[i]);
             stmt.setInt(2,0);
             stmt.setInt(3,0);
             stmt.setFloat(4,0);
             stmt.setFloat(5,0);
            ...
            for(i=0;i<n;i++)
            {
            stmt=connection.con.prepareStatement("delete from "+"scorecardbat"+brd);
            stmt.executeUpdate();}
            int m=0;
            ...
            for(i=0;i<m;i++)
            {
             stmt=connection.con.prepareStatement("delete from "+"scorecardball"+brd);
             stmt.executeUpdate();}
