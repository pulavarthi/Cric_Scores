...
public class teams extends javax.swing.JFrame implements ActionListener {
    ...
    teams()
    {
            ...
            bpp = new JButton("PLAYER PROFILE");
            ...
            try{
            cb1 = new JComboBox<Object>(cn.teamsare);
    		...
    		System.out.println("error in combo box");
    	}
    	
    	
            j1=new JFrame("Teams page");
            j1.setLayout(null);
            j1.setSize(715, 950);
            j1.setVisible(true);
     ...
     void team()  
    {
        ...
        try{
            stmt=connection.con.prepareStatement("select count(*) from "+tm);
            rs=stmt.executeQuery();
            while(rs.next())
            p=rs.getInt("count(*)");
           for(i=1;i<=p;i++){
               nm[i]=new JRadioButton();
         stmt=connection.con.prepareStatement("Select * from "+tm+" where SR=?");
         stmt.setInt(1, i);
         rs=stmt.executeQuery();
           while(rs.next())
     ...
     void change()
     {
         ...
         try{
             for(i=0;i<2;i++)
             {
             stmt=connection.con.prepareStatement("select * from "+tm+" where PLAYER=?");
         ...
         stmt=connection.con.prepareStatement("update "+tm+" set SR=? where PLAYER=?");
        stmt.setInt(1, n[1]);
     ...
     void addteam()
     {
         ...
         try {
             stmt=connection.con.prepareStatement("select * from teams where TEAMS=?");
             stmt.setString(1, s);
            rs=stmt.executeQuery();
            if(rs.absolute(1))
          ...
          String Table = "CREATE TABLE "+s+
              "  (SR INT(2),"+ "PLAYER VARCHAR(30),"+ "TESTBAT INT(5),"
            ...
            + "AGE INT(3))";//+"primary key(SR))";  
            st = connection.con.createStatement();
            st.executeUpdate(Table);
            stmt=connection.con.prepareStatement("insert into teams values(?)");
            stmt.setString(1, s);
            stmt.executeUpdate();
